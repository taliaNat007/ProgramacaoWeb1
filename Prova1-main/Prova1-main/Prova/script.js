...
document.addEventListener('DOMContentLoaded', () => {
    ...
    btnCancel.addEventListener('clik', resetForm);
    ...
    function loadItems() {
        fetch(apiUrl)
            ...
            .catch(error =>
                console.error('Error ao carregar os itens', error)); // Exibe o erro no console
    }
    ...
    function addItemToList(item) {
        ...
        li.querySelector('.btn-edit').addEventListener('click', () => editItem(item.id));
    ...
    function editItem(item) {
        ...
        // Verifica se o botão de cancelar existe e remove a classe 'hidden' para torná-lo visível
        if (btnCancel) {
    ...
    function deleteItem(id) {
        if (confirm('Tem certeza que deseja excluir este item?')) { // Confirmação
            fetch(`${apiUrl}/${id}`, { method: 'DELETE' }) // Requisição DELETE
                ...
                .catch(error => {
                    console.error('Erro ao excluir o item:', error); // Exibe o erro no console
                    alert('Ocorreu um erro ao excluir o item. Tente novamente.'); // Mensagem de erro
                });
        }
    }
